from src.rota.rotas import remove_user_from_rota_on_date
...
def test_remove_user_from_rota_on_date_fails_with_future_snapshots(
    ...
    with custom_db(custom_db_specification) as db:
        with pytest.raises(ValueError):
            remove_user_from_rota_on_date(user_id=0, rota_id=0, date=new_date, db=db)


def test_remove_user_from_rota_on_date_fails_if_user_not_in_rota(
    custom_db: DBFactory,
) -> None:
    user_table = {NEW_USER_NAME: 0, "other_user": 1}
    custom_db_specification: DBSpecification = {
        INITIAL_ROTA_NAME: [
            {"date": INITIAL_SNAPSHOT_DATE, "user_list": [NEW_USER_NAME]}
        ]
    }
    with custom_db(custom_db_specification, user_table=user_table) as db:
        with pytest.raises(ValueError):
            remove_user_from_rota_on_date(
                user_id=1,
                rota_id=0,
                date=INITIAL_SNAPSHOT_DATE + timedelta(days=7),
                db=db,
            )
...
@pytest.mark.parametrize(
    "initial_user_list, final_user_list",
    [
        # (["user_1"], []),
        (["user_1", "user_2"], ["user_2"]),
        (["user_1", "user_2", "user_3"], ["user_2", "user_3"]),
    ],
    ids=["two_users", "three_users"],
)
def test_remove_user_from_rota_on_date(
    offset: int,
    expected_cycle: int,
    initial_user_list: list[str],
    final_user_list: list[str],
    custom_db: DBFactory,
) -> None:
        ...
        custom_db_specification["rota_1"].append(
            {
                ...
                "user_list": cycle_list(final_user_list, expected_cycle),
            }
        )
